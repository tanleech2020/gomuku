...
def checkWin (row, col, isPlayer):
    ...
    if (row >= 8 and col >=8) or (row<=0 and col<=0):
        ...
        return check_horizontal_win(row,col,piece) or check_vertical_win(row,col,piece) or check_diagonal_b_win(row,col,piece)
    elif (row <=0 and col >=8) or (row>=8 and col<=0):
        # only check foreward diagonal, horizontal and vertical
        return check_horizontal_win(row,col,piece) or check_vertical_win(row,col,piece) or check_diagonal_f_win(row,col,piece)
    else:
        return check_horizontal_win(row,col,piece) or check_vertical_win(row,col,piece) or check_diagonal_f_win(row,col,piece) or check_diagonal_b_win(row,col,piece)
